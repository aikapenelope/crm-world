...
| CRUD forms in admin | `<CrudForm fields={[...]} groups={[...]} initialValues={...} onSubmit={...} cancelHref="/path" />` from `@open-mercato/ui/backend/CrudForm`. **v0.6.1**: `entityId`/`apiPath`/`mode` ya NO son props — ver `docs/PATTERNS.md §CrudForm`. Helpers: `createCrud`/`updateCrud`/`deleteCrud` from `@open-mercato/ui/backend/utils/crud`. Never raw `<form>` or raw `fetch` | `node_modules/@open-mercato/ui/src/backend/CrudForm.tsx` |
| DataTables in admin | `<DataTable entityId={...} columns={...} data={...} />` from `@open-mercato/ui/backend/DataTable`. **v0.6.1**: `apiPath` y `extensionTableId` NO son props válidas — ver `DataTableProps` en `node_modules/@open-mercato/ui/src/backend/DataTable.tsx`. `emptyState` acepta `ReactNode` (string o elemento) | `node_modules/@open-mercato/ui/src/backend/DataTable.tsx` |
