...
| CRUD forms in admin | `<CrudForm fields={[...]} groups={[...]} initialValues={...} onSubmit={async (values) => {}} cancelHref="/path" />` from `@open-mercato/ui/backend/CrudForm`. Props verified against `CrudFormProps` in v0.6.2: `fields: CrudField[]` (required), `groups?: CrudFormGroup[]`, `entityId?: string` (optional, only for custom field definitions), `cancelHref?: string`, `onSubmit?`, `title?`, `submitLabel?`. **No `apiPath` prop, no `mode` prop in this version.** Helpers: `createCrud` / `updateCrud` / `deleteCrud` from `@open-mercato/ui/backend/utils/crud`; `createCrudFormError` from `@open-mercato/ui/backend/utils/serverErrors`. Never raw `<form>` or raw `fetch` | `node_modules/@open-mercato/ui/src/backend/CrudForm.tsx` (CrudFormProps:289) |
| DataTables in admin | `<DataTable columns={[...]} data={[...]} isLoading={bool} emptyState={<ReactNode>} />` from `@open-mercato/ui/backend/DataTable`. Props verified against `DataTableProps` in v0.6.2: `isLoading?: boolean` (not `loading`), `emptyState?: React.ReactNode` (not `emptyTitle`/`emptyDescription`). Keep column definitions stable so widget injection (columns, row actions, filters, toolbar) keeps working | `node_modules/@open-mercato/ui/src/backend/DataTable.tsx` (DataTableProps) |
