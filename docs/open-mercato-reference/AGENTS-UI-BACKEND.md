...
5. **MUST use `useGuardedMutation` when not using `CrudForm`** — wrap every write operation (`POST`/`PUT`/`PATCH`/`DELETE`) in `runMutation({ operation: async () => { ... }, context })` so global mutation injections run consistently. **v0.6.1 API**: `useGuardedMutation({ contextId: 'module.page' })` requiere `contextId`. `operation` es la función async (no string). `mutationPayload` es ahora un objeto de datos opcional (no la función). Ver `node_modules/@open-mercato/ui/src/backend/injection/useGuardedMutation.ts`
