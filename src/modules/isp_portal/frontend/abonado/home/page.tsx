...
export default function IspPortalHome() {
  ...
  if (error || !data) return (
    <div className="max-w-lg mx-auto p-6 text-center">
      ...
      <p className="text-sm text-muted-foreground">Número de cuenta: verifica que tu usuario está correctamente vinculado al ISP.</p>
    </div>
  )
  ...
  return (
    <div className="mx-auto max-w-2xl p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold">Hola, abonado</h1>
          <p className="text-sm text-muted-foreground mt-1">Cuenta {subscriber.account_number} · {subscriber.installation_city}</p>
        </div>
      ...
      <div className={`rounded-xl border p-6 ${hasDebt ? 'border-status-error-border bg-status-error-bg' : 'border-status-success-border bg-status-success-bg'}`}>
        ...
        {hasDebt && (
          <Button type="button" className="mt-4" onClick={() => router.push('/abonado/facturas')}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          type="button"
          onClick={() => router.push('/abonado/facturas')}
        ...
        <button
          type="button"
          onClick={() => router.push('/abonado/soporte')}
        ...
        <button
          type="button"
          onClick={() => router.push('/abonado/servicio')}
          className="flex flex-col items-center gap-2 rounded-lg border border-border p-4 hover:bg-accent transition-colors text-sm font-medium"
        >
        ...
        <button
          type="button"
          onClick={() => router.push('/abonado/soporte/nuevo')}
          className="flex flex-col items-center gap-2 rounded-lg border border-border p-4 hover:bg-accent transition-colors text-sm font-medium"
        >
      ...
      {subscriber.last_payment_date && (
        <p className="text-xs text-center text-muted-foreground">
          Último pago registrado: {new Date(subscriber.last_payment_date).toLocaleDateString('es-VE')}
          {' · '}Próximo vencimiento: día {subscriber.billing_cycle_day} de cada mes
        </p>
      )}
    </div>
  )
