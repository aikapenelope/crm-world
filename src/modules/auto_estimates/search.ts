  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: lines, presenter: { title: (norm(r.estimate_number) ?? undefined) ?? 'Presupuesto', subtitle: (norm(r.status) ?? undefined) ?? undefined, icon: 'file-text', badge: (norm(r.status) ?? undefined) ?? undefined }, links: [{ href: '/backend/auto_estimates', label: 'Ver presupuestos', kind: 'primary' }], checksumSource: { estimate_number: r.estimate_number, status: r.status, updated_at: r.updated_at } }
      },
