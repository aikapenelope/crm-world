  ...
  const columns: ColumnDef<OrderRow>[] = [
    {
      ...
      header: 'Orden',
      cell: ({ row }) => <span className="font-mono font-bold text-sm">{row.original.order_number}</span>,
    },
    {
      accessorKey: 'received_at',
      header: 'Ingreso',
      cell: ({ row }) => new Date(row.original.received_at).toLocaleDateString('es-VE'),
    },
    {
      accessorKey: 'status',
      header: 'Status',
    ...
    {
      accessorKey: 'customer_complaint',
      header: 'Motivo',
      cell: ({ row }) => (
        <span className="max-w-[200px] truncate block text-xs">
          {row.original.customer_complaint ?? '—'}
        </span>
      ),
    },
    {
      ...
      header: 'Total',
      cell: ({ row }) => {
        const total = Number(row.original.total_amount)
        return total > 0 ? `${row.original.currency} ${total.toLocaleString('es-VE', { minimumFractionDigits: 2 })}` : '—'
      },
    },
  ]
