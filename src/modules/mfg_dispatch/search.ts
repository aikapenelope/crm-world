  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.order_number) ?? '', norm(r.customer_name) ?? ''].filter(Boolean), presenter: { title: (norm(r.order_number) ?? undefined) ?? 'SO', subtitle: `${norm(r.customer_name) ?? ''} · USD ${r.total_usd}`, icon: 'truck', badge: SO_STATUS[String(r.status ?? '')] ?? '' }, links: [{ href: `/backend/mfg-dispatch/${r.id}`, label: 'Ver pedido', kind: 'primary' }], checksumSource: { order_number: r.order_number, status: r.status, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.order_number) ?? undefined) ?? 'SO', subtitle: (norm(r.customer_name) ?? undefined) ?? undefined, icon: 'truck', badge: SO_STATUS[String(r.status ?? '')] ?? '' } },
