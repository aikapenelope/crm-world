...
export const searchConfig: SearchModuleConfig = {
  entities: [
    {
      entityId: 'academy_enrollments.enrollment',
      enabled: true,
      priority: 30,
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return {
          text: lines,
          presenter: {
            title: (norm(r.student_name) as string | undefined) ?? 'Alumno',
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        ...
        return {
          title: (norm(r.student_name) as string | undefined) ?? 'Alumno',
          subtitle: (norm(r.enrollment_number) as string | undefined) ?? undefined,
          icon: 'user-check',
          badge: statusLabel,
        }
      },
