  ...
  entities: [
    {
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.report_number as string) as string | undefined) ?? 'Turno', subtitle: (norm(r.work_center_name as string) as string | undefined) ?? undefined, icon: 'activity', badge: r.shift_type } },
