  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.report_number) ?? '', norm(r.work_center_name) ?? ''].filter(Boolean), presenter: { title: (norm(r.report_number) as string | undefined) ?? 'Turno', subtitle: `OEE: ${r.oee_total_pct}% total / ${r.oee_internal_pct}% interno`, icon: 'activity', badge: r.shift_type }, links: [{ href: '/backend/mfg-floor', label: 'Ver dashboard', kind: 'primary' }], checksumSource: { report_number: r.report_number, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.report_number) as string | undefined) ?? 'Turno', subtitle: (norm(r.work_center_name) as string | undefined) ?? undefined, icon: 'activity', badge: r.shift_type } },
