  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        const presenter: SearchResultPresenter = {
          title: (norm(r.inspection_number) ?? undefined) ?? 'Inspection Number',
          subtitle: ((norm(r.overall_condition) ?? '')).replace(/^\s*·\s*|\s*·\s*$/g, '') || undefined,
          icon: 'clipboard-check',
          badge: (norm(r.overall_condition) ?? undefined) ?? undefined,
        }
        return {
          text: lines,
          presenter,
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        ...
        return {
          title: (norm(r.inspection_number) ?? undefined) ?? 'Inspection Number',
          subtitle: (norm(r.overall_condition) ?? undefined) ?? undefined,
          icon: 'clipboard-check',
          badge: (norm(r.overall_condition) ?? undefined) ?? undefined,
        }
      },
