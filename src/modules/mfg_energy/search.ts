  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        const r = ctx.record
        return { text: [OUTAGE_TYPE[String(r.outage_type ?? '')] ?? String(r.outage_type ?? ''), norm(r.zone as string) ?? ''].filter(Boolean), presenter: { title: `Corte ${new Date(String(r.started_at ?? \'\')).toLocaleDateString()}`, subtitle: `${OUTAGE_TYPE[String(r.outage_type ?? '')] ?? ''} · ${r.duration_hrs ? `${r.duration_hrs}h` : 'en curso'}`, icon: 'zap-off', badge: r.ended_at ? 'Terminado' : 'Activo' }, links: [{ href: '/backend/mfg-energy', label: 'Ver energía', kind: 'primary' }], checksumSource: { started_at: r.started_at, ended_at: r.ended_at, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: `Corte ${new Date(String(r.started_at ?? \'\')).toLocaleDateString()}`, subtitle: (norm(r.zone as string) as string | undefined) ?? undefined, icon: 'zap-off', badge: r.ended_at ? 'Terminado' : 'Activo' } },
