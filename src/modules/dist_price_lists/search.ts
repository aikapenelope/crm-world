  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [String(r.name), norm(r.code) ?? '', norm(r.description) ?? ''].filter(Boolean), presenter: { title: norm(r.name) ?? 'Lista de precios', subtitle: norm(r.code) ?? undefined, icon: 'tag' }, links: [{ href: '/backend/dist_price_lists', label: 'Ver listas', kind: 'primary' }], checksumSource: { name: r.name, code: r.code, updated_at: r.updated_at } }
      },
