  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.po_number) ?? '', norm(r.supplier_name) ?? ''].filter(Boolean), presenter: { title: (norm(r.po_number) as string | undefined) ?? 'OC', subtitle: `${norm(r.supplier_name) ?? ''} · CIF: ${r.total_cif_cost}`, icon: 'shopping-cart', badge: statusLabel }, links: [{ href: `/backend/mfg-procurement/${r.id}`, label: 'Ver OC', kind: 'primary' }], checksumSource: { po_number: r.po_number, status: r.status, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.po_number) as string | undefined) ?? 'OC', subtitle: (norm(r.supplier_name) as string | undefined) ?? undefined, icon: 'shopping-cart', badge: PO_STATUS[String(r.status ?? '')] ?? String(r.status ?? '') } },
