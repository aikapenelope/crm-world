    ...
    // =========================================================================
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return {
          text: lines,
          presenter: {
            title: `${priorityIcon} ${norm(r.ticket_number as string) ?? 'Ticket'}`.trim(),
            subtitle: (norm(r.subject as string) as string | undefined) ?? undefined,
            icon: 'message-circle',
            badge: statusLabel,
          },
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        ...
        const statusLabel = STATUS_LABELS[String(r.status ?? '')] ?? String(r.status ?? '')
        return {
          title: (norm(r.ticket_number as string) as string | undefined) ?? 'Ticket',
          subtitle: (norm(r.subject as string) as string | undefined) ?? undefined,
          icon: 'message-circle',
          badge: statusLabel,
        }
