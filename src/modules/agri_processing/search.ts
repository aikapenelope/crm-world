  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return {
          ...
          presenter: { title: norm(r.batch_number) ?? 'Beneficio', subtitle: new Date(r.slaughter_date as string).toLocaleDateString('es-VE'), icon: 'factory', badge: BATCH_STATUS[String(r.status)] ?? String(r.status) },
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        const r = ctx.record
        return { title: norm(r.batch_number) ?? 'Beneficio', subtitle: BATCH_STATUS[String(r.status)] ?? String(r.status), icon: 'factory', badge: BATCH_STATUS[String(r.status)] ?? String(r.status) }
      },
    ...
    },
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return {
          ...
          presenter: { title: norm(r.lot_number) ?? 'Producto', subtitle: norm(r.barcode) ?? undefined, icon: 'package', badge: LOT_STATUS[String(r.status)] ?? String(r.status) },
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        const r = ctx.record
        return { title: norm(r.lot_number) ?? 'Producto', icon: 'package', badge: LOT_STATUS[String(r.status)] ?? String(r.status) }
      },
