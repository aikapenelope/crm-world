  ...
  entities: [
    {
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: `${norm(r.equipment_code as string)} — ${norm(r.name as string) ?? ''}`, subtitle: (norm(r.work_center_name as string) as string | undefined) ?? undefined, icon: 'tool', badge: r.status } },
