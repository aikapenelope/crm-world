...
export const searchConfig: SearchModuleConfig = {
  entities: [
    {
      entityId: 'mfg_costs.mfg_cost_variance',
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.order_number) ?? '', norm(r.product_code) ?? ''].filter(Boolean), presenter: { title: (norm(r.order_number) ?? undefined) ?? 'Orden', subtitle: `Variación: USD ${variance.toFixed(2)} (${variance > 0 ? 'desfavorable' : 'favorable'})`, icon: 'dollar-sign', badge: r.status as string | undefined }, links: [{ href: '/backend/mfg-costs', label: 'Ver costos', kind: 'primary' }], checksumSource: { order_number: r.order_number, status: r.status, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.order_number) ?? undefined) ?? 'Variación', subtitle: (norm(r.product_code) ?? undefined) ?? undefined, icon: 'dollar-sign', badge: r.status as string | undefined } },
