...
export const searchConfig: SearchModuleConfig = {
  entities: [
    {
      entityId: 'const_materials.order',
      enabled: true,
      priority: 20,
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        const presenter: SearchResultPresenter = {
          title: (norm(r.order_number as string) as string | undefined) ?? 'Order Number',
          subtitle: ((norm(r.supplier_name as string) ?? '') + ' · ' + (norm(r.status as string) ?? '')).replace(/^\s*·\s*|\s*·\s*$/g, '') || undefined,
          icon: 'package',
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        ...
        return {
          title: (norm(r.order_number as string) as string | undefined) ?? 'Order Number',
          subtitle: (norm(r.supplier_name as string) as string | undefined) ?? undefined,
          icon: 'package',
          badge: (norm(r.status as string) as string | undefined) ?? undefined,
        }
      },
