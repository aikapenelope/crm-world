...
export const searchConfig: SearchModuleConfig = {
  entities: [
    {
      entityId: 'mfg_mrp.purchase_requisition',
      enabled: true,
      priority: 40,
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.requisition_number as string) ?? '', norm(r.material_code as string) ?? ''].filter(Boolean), presenter: { title: (norm(r.requisition_number as string) as string | undefined) ?? 'REQ', subtitle: `${norm(r.material_code as string)} — ${r.quantity} ${r.uom}`, icon: 'shopping-cart', badge: r.status }, links: [{ href: `/backend/mfg-mrp`, label: 'Ver MRP', kind: 'primary' }], checksumSource: { requisition_number: r.requisition_number, status: r.status, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.requisition_number as string) as string | undefined) ?? 'REQ', subtitle: (norm(r.material_code as string) as string | undefined) ?? undefined, icon: 'shopping-cart', badge: r.status } },
