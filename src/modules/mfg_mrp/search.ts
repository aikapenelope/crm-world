  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.requisition_number) ?? '', norm(r.material_code) ?? ''].filter(Boolean), presenter: { title: (norm(r.requisition_number) as string | undefined) ?? 'REQ', subtitle: `${norm(r.material_code)} — ${r.quantity} ${r.uom}`, icon: 'shopping-cart', badge: r.status }, links: [{ href: `/backend/mfg-mrp`, label: 'Ver MRP', kind: 'primary' }], checksumSource: { requisition_number: r.requisition_number, status: r.status, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.requisition_number) as string | undefined) ?? 'REQ', subtitle: (norm(r.material_code) as string | undefined) ?? undefined, icon: 'shopping-cart', badge: r.status } },
