  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [String(r.count_number), String(r.status ?? '')].filter(Boolean), presenter: { title: (norm(r.count_number) ?? undefined) ?? 'Conteo', subtitle: (norm(r.status) ?? undefined) ?? undefined, icon: 'package' }, links: [{ href: '/backend/retail_inventory', label: 'Ver inventario', kind: 'primary' }], checksumSource: { count_number: r.count_number, status: r.status, updated_at: r.updated_at } }
      },
