  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [String(r.name)], presenter: { title: (norm(r.name) as string | undefined) ?? 'Regla de precio', icon: 'percent' }, links: [{ href: '/backend/retail_pricing', label: 'Ver precios', kind: 'primary' }], checksumSource: { name: r.name, updated_at: r.updated_at } }
      },
