  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        const presenter: SearchResultPresenter = {
          title: (norm(r.preferred_city) as string | undefined) ?? 'Preferred City',
          subtitle: ((norm(r.preferred_type) ?? '') + ' · ' + (norm(r.preferred_operation) ?? '')).replace(/^\s*·\s*|\s*·\s*$/g, '') || undefined,
          icon: 'search',
          badge: (norm(r.preferred_operation) as string | undefined) ?? undefined,
        }
        return {
          text: lines,
          presenter,
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        ...
        return {
          title: (norm(r.preferred_city) as string | undefined) ?? 'Preferred City',
          subtitle: (norm(r.preferred_type) as string | undefined) ?? undefined,
          icon: 'search',
          badge: (norm(r.preferred_operation) as string | undefined) ?? undefined,
        }
      },
