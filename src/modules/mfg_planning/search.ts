  ...
  entities: [
    {
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.schedule_number as string) as string | undefined) ?? 'MPS', subtitle: (norm(r.product_code as string) as string | undefined) ?? undefined, icon: 'calendar', badge: r.status as string | undefined } },
