  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.schedule_number) ?? '', norm(r.product_code) ?? ''].filter(Boolean), presenter: { title: norm(r.schedule_number) ?? 'MPS', subtitle: `${norm(r.product_code)} · ${r.planned_quantity} ${r.uom}`, icon: 'calendar', badge: r.status }, links: [{ href: '/backend/mfg-planning', label: 'Ver MPS', kind: 'primary' }], checksumSource: { schedule_number: r.schedule_number, status: r.status, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: norm(r.schedule_number) ?? 'MPS', subtitle: norm(r.product_code) ?? undefined, icon: 'calendar', badge: r.status } },
