  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: lines, presenter: { title: (norm(r.supplier_name) as string | undefined) ?? 'Retención', subtitle: (norm(r.period_month) as string | undefined) ?? undefined, icon: 'file-text', badge: (norm(r.period_month) as string | undefined) ?? undefined }, links: [{ href: '/backend/ve_withholdings', label: 'Ver retenciones', kind: 'primary' }], checksumSource: { supplier_name: r.supplier_name, invoice_number: r.invoice_number, period_month: r.period_month } }
      },
