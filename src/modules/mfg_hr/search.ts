  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        return { text: [norm(r.full_name as string) ?? '', norm(r.employee_code as string) ?? ''].filter(Boolean), presenter: { title: `${norm(r.employee_code as string)} — ${norm(r.full_name as string) ?? ''}`, subtitle: `${r.shift_type} · ${r.work_center_name ?? ''}`, icon: 'user', badge: r.is_active ? 'Activo' : 'Inactivo' }, links: [{ href: '/backend/mfg-hr', label: 'Ver RRHH', kind: 'primary' }], checksumSource: { full_name: r.full_name, shift_type: r.shift_type, updated_at: r.updated_at } }
      },
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.full_name as string) as string | undefined) ?? 'Operario', subtitle: (norm(r.employee_code as string) as string | undefined) ?? undefined, icon: 'user', badge: r.shift_type as string | undefined } },
