  ...
  entities: [
    {
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => { const r = ctx.record; return { title: (norm(r.full_name as string) as string | undefined) ?? 'Operario', subtitle: (norm(r.employee_code as string) as string | undefined) ?? undefined, icon: 'user', badge: r.shift_type } },
